...
class PodSignOutStoreTest : SempodsIntegrationTest() {
  ...
  fun `a later sign-out moves the instant and an earlier one does not`() {
    ...
    store.record(podId.toPodId(), listOf(person), now)
    store.record(podId.toPodId(), listOf(person), now.minusSeconds(60))
    assertEquals(now, store.signedOutAt(podId.toPodId(), listOf(person)))

    store.record(podId.toPodId(), listOf(person), now.plusSeconds(60))
    assertEquals(now.plusSeconds(60), store.signedOutAt(podId.toPodId(), listOf(person)))
  }

  @Test
  fun `an instant belongs to one pod and to the URIs it was written for`() {
    store.record(podId.toPodId(), listOf(person, twin), now)

    assertEquals(now, store.signedOutAt(podId.toPodId(), listOf(twin)))
    assertNull(store.signedOutAt(otherPodId.toPodId(), listOf(person)), "another pod")
    assertNull(store.signedOutAt(podId.toPodId(), listOf("https://id.test/e/somebody-else")), "another person")
    assertNull(store.signedOutAt(podId.toPodId(), emptyList()))
  }
  ...
  fun `deleteByPod removes exactly that pod's rows`() {
    store.record(podId.toPodId(), listOf(person, twin), now)
    store.record(otherPodId.toPodId(), listOf(person), now)

    assertEquals(2L, store.deleteByPod(podId.toPodId()))
    assertNull(store.signedOutAt(podId.toPodId(), listOf(person, twin)))
    assertEquals(now, store.signedOutAt(otherPodId.toPodId(), listOf(person)))
  }
