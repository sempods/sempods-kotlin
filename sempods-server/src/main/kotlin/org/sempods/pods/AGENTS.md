...
| `InMemoryPodRepository` | **Writes** go write-through against the MemoryStore (source of truth on write), then the backup sink mirrors the change into MongoDB — see `write-through.md`. **Reads** (`getResource`, `existsResource`, `findReferencingResources`) come from the MemoryStore. Domain listings (an event listing, say) are not here — they live in the consuming application as SPARQL-native queries. |
