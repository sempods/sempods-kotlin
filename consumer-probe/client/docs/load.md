...
The manual [load harness](../src/load/java/org/sempods/probe/client/ClientLoad.java) compares
...
```sh
./gradlew :consumer-probe:client:load21 --console=plain
./gradlew :consumer-probe:client:load25 --console=plain
```
...
`consumer-probe/client/build/load/java-<version>/`. Preserve both with the issue/PR evidence.
...
```sh
./gradlew :consumer-probe:client:load21 -PloadOutput=/tmp/sempods-load/java-21 --console=plain
./gradlew :consumer-probe:client:load25 -PloadOutput=/tmp/sempods-load/java-25 --console=plain
./gradlew :consumer-probe:client:load21 -PloadWorkloads=consumer -PloadWarmup=5 -PloadSeconds=15 -PloadOutput=/tmp/sempods-load/java-21-consumer-long --console=plain
./gradlew :consumer-probe:client:load21 -PloadWorkloads=slow -PloadWarmup=5 -PloadSeconds=15 -PloadOutput=/tmp/sempods-load/java-21-slow-long --console=plain
```
