...
The isolation review covers: every persisted collection and every
...
WebID `user`; the finer isolation bundle is `(user, profile)`, and everything
connection/token-shaped is keyed `(user, profile, pod)` (`persist/PodKey.kt`).

[Review snapshot](https://github.com/sempods/sempods-kotlin/blob/f056eccaf7451d34aaa4577905bf72c5920d1e74/sempods-mcp/docs/multi-tenancy-review.md). Reviewed against the then-current DAOs;
[runtime documentation](runtime.md) owns current behavior. A new collection or query
...
| `auditLog` | `AuditLogDao` | index `(user, profile, ts)` | ✓ append-only; the only read (`listFor`) requires `user` — there is deliberately no unscoped listing on the DAO. Rows carry no token material, no request bodies, no tool arguments (fixed `detail` labels only). TTL-bounded via `expiresAt`. |
...
| `oauth.loginStates`, `oauth.consentTransactions`, `oauth.webLoginStates` | One-time stores | SHA-256 `_id` of the one-time secret | ✓ same pattern: secret-keyed, single-use consume, TTL-indexed; the consumer re-binds the entry to the session where a session exists. |
| `oauth.reauthChallenges` | `ReauthorizeChallengeStore` (`:sempods-mcp-core`) | `_id` = `(profile, clientId, user)` | ✓ the odd one out of these stores: its `_id` is not a hash, because nothing here is redeemable — the row gates a loop-avoidance shortcut and mints nothing. Single-use consume is still atomic, and the replay predicate re-binds to the token (`jti` must differ, `iat` at-or-after the challenge). The profile is part of the replay boundary. |
...
| `oauth.signingKeys` | `SigningKeyDao` | service-global | ✓ by design: one signing key set for all profiles/users (the per-profile issuer is a claim/URL split, not a key split). Private JWK encrypted at rest; bootstrap is race-safe. |
...
| `PodTokenProvider.locks` (per-key refresh mutex) | `PodKey` | **Finding, fixed in this review** — see below. |
...
- **MCP endpoint (profile isolation):** a bearer's `profile` claim must match the path it
  arrived on; `iss` is bound to the profile. The per-user quota sits *after* these gates,
...
- **Service AS:** authorization codes and refresh tokens are profile-bound and re-validated
  against the endpoint path; rotation reuse revokes the family (audited).
...
1. **`PodTokenProvider.locks` grew unbounded** — one `Mutex` per `(user, profile, pod)` ever
   ...
   currently-unlocked mutexes. Safety: the correctness primitive against
...
- [`../AGENTS.md`](../AGENTS.md) — module conventions; [runtime documentation](runtime.md) owns current behavior.
