...
regardless of access-token expiry, which no refresh token's lifetime runs on. The default is a
conservative guess; the KDoc on `SempodsMcpConfig.podTokenFamilyPreserveSeconds` owns why, and what
happens when a pod ends the grant sooner. "Used" is a throttled `lastUsedAt` on the vault row,
