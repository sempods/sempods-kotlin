 ...
 * |---|---|---|---|
 * | `result=granted&scope=…` | true | what the owner granted, at least one scope and possibly fewer than were asked for | null |
...
class SempodsGrantOutcome private constructor(
  ...
  companion object {
     ...
     *   or when the query carries neither `result=granted` with a scope nor `error`.
     */
    @JvmStatic
    ...
    fun readQuery(encodedQuery: String?, expectedState: String): SempodsGrantOutcome {
      ...
      val granted = scopesOf(query.single("scope"))
      return when {
        // The pod answers a consent that granted nothing `access_denied`, so an empty grant is not its answer.
        result == "granted" && error == null && granted.isNotEmpty() -> SempodsGrantOutcome(granted, error = null, errorDescription = null)
        result == null && error != null -> SempodsGrantOutcome(emptySet(), error, query.single("error_description"))
        else -> throw query.refused("carries neither a granted result with its scopes nor an error")
      }
    }
  }
}
