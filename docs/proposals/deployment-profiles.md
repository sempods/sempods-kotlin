...
| **Transport without RDF** | Let a consumer of the host-level admin surface take the HTTP plumbing without the pod client — an operator console with no RDF anywhere. | The plumbing exists: `:sempods-client-core` is HTTP against a pod with no RDF4J, Jena or Jackson 2 in its graph. What still carries them is `:sempods-control-plane-client`, which declares `api(project(":sempods-client"))` and so hands a consumer `SempodsHttpTransport` and `SempodsClientException` with RDF4J attached; the same holds for the hosted MCP consumer. Moving both onto the core is [#152](https://github.com/sempods/sempods-kotlin/issues/152). See [`../pod-client.md`](../pod-client.md). |
