...
| **Transport without RDF** | Let a consumer of the host-level admin surface take the HTTP plumbing without the pod client — an operator console with no RDF anywhere. | Closed for the admin surface: `:sempods-control-plane-client` runs on `:sempods-client-core` directly, so an operator console resolves no RDF library, no legacy transport and no Jackson 2. The hosted MCP consumer still reaches its pods through `:sempods-client`'s transport — [#239](https://github.com/sempods/sempods-kotlin/issues/239) and [#241](https://github.com/sempods/sempods-kotlin/issues/241) move it. See [`../pod-client.md`](../pod-client.md). |
