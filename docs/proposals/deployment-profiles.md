...
| **Transport without RDF** | Let a consumer of the host-level admin surface take the HTTP plumbing without the pod client — an operator console with no RDF anywhere. | Largely closed: `:sempods-client-core` is HTTP against a pod with no RDF library in its graph, and `:sempods-client` carries none either since the legacy tiers went, so `:sempods-control-plane-client` and the hosted MCP consumer no longer inherit one through it. What they still inherit is the legacy transport and its Jackson 2 — moving both onto the core is [#152](https://github.com/sempods/sempods-kotlin/issues/152). See [`../pod-client.md`](../pod-client.md). |
