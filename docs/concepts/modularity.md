...
module hides — the HTTP core no RDF or JSON library, an RDF adapter nothing, since RDF4J is what it
is for. OkHttp is not on the client core's list: it names the engine on purpose, on `api`.
