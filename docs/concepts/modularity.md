...
an entry in the root build that also names the libraries it hides — the client core no RDF or JSON
library; OkHttp is on its surface on purpose. Most modules do not satisfy the rule yet, and
[#15](https://github.com/sempods/sempods-kotlin/issues/15) owns that.
...
resolve RDF4J, Jena, Jackson or the legacy DTOs — a question dependency analysis cannot answer,
