...
dependencies {
  ...
  // `api`, and that is this module's shape rather than an oversight. `SempodsSession` hands out an
  // `okhttp3.Request.Builder` and `SempodsOkHttp` configures an `okhttp3.OkHttpClient.Builder`; a
  // consumer compiles against both, adds its own interceptor, shares the connection pool. Hiding the engine behind a second vocabulary
