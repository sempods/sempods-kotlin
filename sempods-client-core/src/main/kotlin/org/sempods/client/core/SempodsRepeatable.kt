 ...
 *
 * [SempodsSession.execute] sends a request once more after a connection lost before any response,
 * but only for an idempotent method: a POST may already have been acted on, and repeating it would
 * be a duplicate write. RFC 9110 §9.2.2 lets a client repeat a POST when it knows the request is
 * safe for that resource, and this is how the caller says it knows.
 *
 * ```java
 * var query = session.newRequest("POST", sparqlPath).post(queryBody);
 * try (Response response = session.execute(SempodsRepeatable.mark(query).build())) {
 *   ...
 * }
 * ```
 *
